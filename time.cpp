...
Time::Time(const unsigned int year, const unsigned long month, const unsigned long long day,
           const unsigned long long hour, const unsigned long long minute) {
    m_year = year - 1;
    m_month = month - 1;
    m_day = day - 1;
    m_hour = hour - 1;
...
const bool Time::operator==(const Time &other) const {
    if (m_year == other.m_year && m_month == other.m_month && m_day == other.m_day && m_hour == other.m_hour &&
...
ostream &operator<<(ostream &stream, const Time &time) {
    return stream << time.m_year + 1 << ":" << time.m_month + 1 << ":" << time.m_day + 1 << " "
                  << time.m_hour << ":" << time.m_minute << " ";
